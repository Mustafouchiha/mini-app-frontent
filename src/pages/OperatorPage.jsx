...
const formatUzPhone = (v) => {
  const c = phoneCore(v);
  if (c.length !== 9) return v || "—";
  return `+998 ${c.slice(0,2)} ${c.slice(2,5)} ${c.slice(5,7)} ${c.slice(7,9)}`;
};
const reqStatusColor = (s) => ({
  active:"#28A869", hidden:"#D4920A", deleted:"#FF4D4F",
  pending_payment:"#2563EB", pending_approval:"#7C3AED",
}[s] || C.textMuted);
  ...
  return (
    <div style={{ display:"flex", background:C.card, borderRadius:16, padding:4,
      ...
      {[["users",<Users size={13}/>,"Userlar"],
        ...
        ["req_posts",<Package size={13}/>,"Postlar"]].map(([t,icon,lbl])=>(
        <button key={t} onClick={()=>onChange(t)} style={{
          flex:1, padding:"9px 0", borderRadius:12, border:"none", cursor:"pointer",
          fontFamily:"inherit", fontSize:11, fontWeight:700,
          background:active===t?C.primaryDark:"transparent",
      ...
      <div style={{ background:C.card, borderRadius:20, padding:"24px 20px", maxWidth:340, width:"100%" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:"11px", borderRadius:12,
            ...
            fontFamily:"inherit", fontSize:13, fontWeight:700, color:C.textSub }}>Bekor</button>
          <button onClick={onConfirm} style={{ flex:1, padding:"11px", borderRadius:12,
            ...
            fontFamily:"inherit", fontSize:13, fontWeight:700 }}>Tasdiqlash</button>
        </div>
      </div>
    </div>
  );
...
function InfoRow({ icon, label, value, copyable }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
      ...
      {copyable && value && (
        <button onClick={()=>copy(value)} style={{ background:"none", border:"none",
          cursor:"pointer", padding:2, color:C.textMuted }}><Copy size={10}/></button>
      )}
    </div>
  );
}
...
export default function OperatorPage({ onBack }) {
  ...
  useEffect(() => {
    operatorAPI.checkMainAdmin().then(d => setIsMainAdmin(d.isMainAdmin)).catch(()=>{});
  }, []);

  // ── Ma'lumot yuklash (showSpinner: birinchi yuklash uchun)
  const load = useCallback(async (q = "", showSpinner = false) => {
    ...
    try {
      ...
      if (currentTab === "users") {
        ...
        setReqPosts(!s ? data : data.filter(p =>
          String(p.title||"").toLowerCase().includes(s) || String(p.id||"").includes(s)
        ));
      }
    } catch (e) {
  ...
  // Tab o'zgarganda — spinner bilan yuklash
  useEffect(() => {
    setQuery("");
    setMsg("");
    load("", true);

    // Fon yangilanishi (spinner yo'q)
    if (intervalRef.current) clearInterval(intervalRef.current);
    intervalRef.current = setInterval(() => {
      load(queryRef.current, false);
    }, 15000);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setConfirm(null);
      load(query, false);
    } catch (e) { setMsg(e.message); setConfirm(null); }
  ...
  const toggleUserBlock = async (u) => {
    try {
      setActionLoading(true);
      const r = await operatorAPI.setUserBlocked(u.id, !u.is_blocked);
      setMsg(`✅ ${r.message}`);
      load(query, false);
  ...
  const toggleOperator = async (u) => {
    try {
      setActionLoading(true);
      const r = await operatorAPI.setUserOperator(u.id, !u.is_operator);
      setMsg(`✅ ${r.message}`);
      load(query, false);
    } catch (e) { setMsg(e.message); }
    finally { setActionLoading(false); }
  ...
  const toggleProductActive = async (p) => {
    try {
      setActionLoading(true);
      const r = await operatorAPI.setProductActive(p.id, !p.is_active);
      setMsg(`✅ ${r.message}`);
      load(query, false);
    } catch (e) { setMsg(e.message); }
    finally { setActionLoading(false); }
  ...
  return (
    <div style={{ fontFamily:"'Nunito','Segoe UI',sans-serif", background:C.bg,
                  minHeight:"100vh", maxWidth:430, margin:"0 auto", padding:"16px 16px 80px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:18 }}>
        ...
        {isMainAdmin && (
          <div style={{ marginLeft:"auto", fontSize:10, background:"#7C3AED", color:"white",
            padding:"3px 8px", borderRadius:8, fontWeight:800 }}>BOSH ADMIN</div>
        )}
      </div>
      ...
      <SearchBar value={query} onChange={setQuery}
        placeholder={tab==="users"?"Ism, telefon, ID, Telegram...":"Nomi, ID, egasi..."}/>

      {/* Spinner — faqat birinchi yuklashda */}
      {loading && (
          ...
          {users.map(u=>(
            <div key={u.id} style={{ background:C.card, borderRadius:16, marginBottom:12,
              ...
              <div style={{ display:"flex", gap:6, marginBottom:8, flexWrap:"wrap" }}>
                ...
                {u.is_operator&&(
                  <span style={{ fontSize:10, fontWeight:800, padding:"2px 8px", borderRadius:6,
                    background:"#EDE9FE", color:"#7C3AED" }}>⚙️ OPERATOR</span>
                )}
              </div>
              ...
              <InfoRow icon={<Wallet size={11}/>}   label="Balans"  value={`${Number(u.balance||0).toLocaleString()} so'm`}/>
              {u.tg_chat_id&&<InfoRow icon={<Hash size={11}/>} label="TG Chat ID" value={String(u.tg_chat_id)} copyable/>}
              <InfoRow icon={<Calendar size={11}/>} label="Sana"
              ...
              <div style={{ display:"flex", gap:6, marginTop:12, flexWrap:"wrap" }}>
                <button onClick={()=>{setDeposit({phone:u.phone,name:u.name,mode:"add"});setAmount("");setMsg("");}}
                  style={{ flex:1, minWidth:55, padding:"8px 4px", borderRadius:10, border:"none",
                ...
                {isMainAdmin&&(
                  <button onClick={()=>toggleOperator(u)} disabled={actionLoading}
                    style={{ flex:1, minWidth:55, padding:"8px 4px", borderRadius:10, border:"none",
                      ...
                      cursor:"pointer", fontSize:11, fontWeight:700,
                      display:"flex", alignItems:"center", justifyContent:"center", gap:3 }}>
                    {u.is_operator?<><ShieldOff size={13}/>Olish</>:<><ShieldCheck size={13}/>Op</>}
                  </button>
                )}
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, color:C.primaryDark, fontWeight:700 }}>
                    {Number(p.price).toLocaleString()} so'm/{p.unit}</div>
                  <div style={{ fontSize:11, color:C.textMuted, marginTop:2 }}>
                    👤 {p.owner_name} ({p.owner_public_id||"—"})</div>
                  <div style={{ fontSize:11, color:C.textMuted }}>
                    📞 {formatUzPhone(p.owner_phone)}</div>
                  <div style={{ fontSize:10, fontWeight:800, marginTop:3,
                    color:p.is_active?"#28A869":C.danger }}>
                    {p.is_active?"✅ FAOL":"🔒 YOPIQ"}</div>
                </div>
            ...
            <div key={p.id} style={{ background:C.card, borderRadius:14, marginBottom:10,
              ...
              <div style={{ display:"flex", justifyContent:"space-between", gap:8 }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:C.primaryDark, fontWeight:700 }}>
                    {Number(p.price).toLocaleString()} so'm</div>
                </div>
              ...
              <div style={{ display:"flex", gap:6, marginTop:10, flexWrap:"wrap" }}>
                ...
                <button onClick={()=>setConfirm({type:"req_post",id:p.id,name:p.title})} disabled={actionLoading}
                  style={{ border:"none", borderRadius:8, padding:"6px 12px",
                    background:"#FFF1F0", color:"#FF4D4F", cursor:"pointer", fontSize:12, fontWeight:700 }}>
                  🗑 O'chirish</button>
              </div>
          ...
          <div style={{ background:C.card, borderRadius:20, padding:"24px 20px", width:"100%", maxWidth:340 }}>
            ...
            {amount&&(
              <div style={{ fontSize:13, color:C.primaryDark, fontWeight:700, marginBottom:14 }}>
                = {Number(amount).toLocaleString()} so'm</div>
            )}
            <div style={{ display:"flex", gap:10 }}>
